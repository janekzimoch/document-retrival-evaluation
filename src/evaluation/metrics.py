...
def get_baseline_performance(ground_truth_rank, rank, test_suites, trials=1000):
    '''
    Parameters: 
           ground_truth_rank: List of results from GPT model.
    ...
    '''
    results_dict_all_trials = {}
    for i in tqdm(range(trials), desc="Generating random trails", leave=False):
        current_seed = i
        random_rank = generate_random_rank(rank, current_seed)
        get_performance_eval(ground_truth_rank, random_rank, results_dict_all_trials, test_suites)
    
    results_dict = reduce_trials_results(results_dict_all_trials, test_suites)
    return results_dict


def reduce_trials_results(results_dict_all_trials, test_suites):
    results_dict = {}
    for test_name in test_suites.keys():
        for k in test_suites[test_name]:
            value = np.mean(results_dict_all_trials[(test_name, k)])
            results_dict[(test_name, k)] = [value]
    return results_dict


def get_performance_eval(ground_truth_rank, random_rank, results_dict, test_suites):
    ' modifies results_dict by reference (in place) '
    for test_name in test_suites.keys():
        for k in test_suites[test_name]:
            value = get_metric(ground_truth_rank, random_rank, test_name, k)
            if (test_name, k) not in results_dict.keys():
                results_dict[(test_name, k)] = [value]
            else:
                results_dict[(test_name, k)].append(value)


def print_results(test_suites, metric_result_dict, model_name):
    '''
    Parameters: 
           test_suites: test metrics
    ...
    for test_name in test_suites.keys():
        for k in test_suites[test_name]:
            value = metric_result_dict[(test_name, k)][0]
            if k != None:
...
def get_metric(gpt_result, method_rank, metric_name, k=None):
    ...
    """
    assert len(gpt_result) == len(
        method_rank), f"{len(gpt_result)}; {len(method_rank)}"
    ...
    for i in gpt_result:
        ...
        curr_original_gpt_result = sort_dict_by_values(
            curr_original_gpt_result)
    ...
    for i in gpt_result:
        ...
        for abs_id in i.keys():
            ...
            curr_relevance_list = [
                c for c in curr_relevance_list if c not in removed_list]
    ...
    if metric_name == "recall":
        ...
        for ind in range(len(method_rank)):
            ...
            if len(curr_relevance_list) == 0:
                continue
            ret_list.append(recall_at_k(
                curr_relevance_list, curr_ranking_result, k))
        ...
        for ind in range(len(method_rank)):
            ...
            if len(curr_relevance_list) == 0:
                continue
            ret_list.append(r_precision(
                curr_relevance_list, curr_ranking_result))
        return np.mean(ret_list) * 100
        ...
        for ind in range(len(method_rank)):
            ...
            ret_list.append(ndcg_normal(
                curr_gpt_rank, curr_gpt_score, curr_ranking_result, p))
        return np.mean(ret_list) * 100
        ...
        for ind in range(len(method_rank)):
            ...
            ret_list.append(
                ndcg_exp(curr_gpt_rank, curr_gpt_score, curr_ranking_result, p))
        return np.mean(ret_list) * 100
        ...
        for ind in range(len(method_rank)):
            ...
            curr_gpt_score = list(original_gpt_result[ind].values())
            curr_ranking_result = method_rank[ind]['index_rank']
            ret_list.append(reciprocal_rank(
                curr_gpt_rank, curr_gpt_score, curr_ranking_result, k))
        return np.mean(ret_list) * 100
        ...
        for ind in range(len(method_rank)):
            ...
            if len(curr_relevance_list) == 0:
                continue
            ret_list.append(average_precision(
                curr_relevance_list, curr_ranking_result))
        return np.mean(ret_list) * 100
        ...
        for ind in range(len(method_rank)):
            ...
            if len(curr_relevance_list) <= 1:
                continue
            curr_gpt_rank = list(original_gpt_result[ind].keys())
            ret_list.append(binary_classification(
                curr_relevance_list, curr_ranking_result, curr_gpt_rank))
        return np.mean(ret_list) * 100

    else:
        raise RuntimeError(
            "metric_name should be one of ['recall', 'r_precision', 'ndcg', 'ndcg_exp', 'mrr', 'map']")
